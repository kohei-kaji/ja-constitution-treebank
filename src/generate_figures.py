...
def main(conllu_path: Path, figure_dir: Path, docname: str) -> None:
    tex_dir = figure_dir / "tex"
    pdf_dir = figure_dir / "pdf"
    png_dir = figure_dir / "png"
    for d in (tex_dir, pdf_dir, png_dir):
    ...
    for sent in sentences:
        num = sent["id"]
        tex_path = tex_dir / f"{docname}-{num}.tex"
        tex_path.write_text(make_tex(sent), encoding="utf-8")
        print(f"  wrote {tex_path}")

    print("\nCompiling PDFs and rendering PNGs...")
    for sent in sentences:
        num = sent["id"]
        stem = f"{docname}-{num}"
        print(f"  lualatex {stem}.tex ...", end=" ", flush=True)
        result = subprocess.run(
            ["lualatex", "-interaction=nonstopmode", f"{stem}.tex"],
            cwd=tex_dir,
            capture_output=True,
            text=True,
        )
        ...
        # move the compiled PDF out of tex_dir into pdf_dir
        pdf_path = pdf_dir / f"{stem}.pdf"
        (tex_dir / f"{stem}.pdf").replace(pdf_path)

        # render the PDF to PNG (single page)
        png_result = subprocess.run(
            ["pdftocairo", "-png", "-r", "300", "-singlefile",
             str(pdf_path), str(png_dir / stem)],
            capture_output=True,
            text=True,
        )
        if png_result.returncode == 0:
            ...
            print("PDF OK, PNG FAILED")
            for line in png_result.stderr.splitlines()[-10:]:
                print("   ", line)

    print("\nCleaning up auxiliary files...")
    for ext in ("*.log", "*.aux"):
        for file_path in tex_dir.glob(ext):
            file_path.unlink(missing_ok=True)
    print("Cleanup finished.")

if __name__ == "__main__":
    # Project root is the parent of this script's directory (src/).
    root_dir = Path(__file__).resolve().parent.parent
    figure_dir = root_dir / "figure"

    main(root_dir / "preamble-gold.conllu", figure_dir, "preamble")
    main(root_dir / "constitution-gold.conllu", figure_dir, "constitution")
